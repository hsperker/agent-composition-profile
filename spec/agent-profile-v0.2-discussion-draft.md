...
Preservation requires a native mechanism with those four properties. Claude Code's `Agent(...)` allowlist, OpenCode's task permission, and agent-as-tool in OpenAI Agents SDK and Microsoft Agent Framework are `preserved`. A project-wide catalog such as Codex's or Copilot CLI's, where the listed agents are available but the list is not enforced per agent, is `resolved`; Copilot's `agents` list is emitted, but the CLI ran an unlisted agent. An adapter tool implementing the four properties on a framework without a relationship primitive is `resolved`. Mechanisms that transfer control (handoffs, workflow transitions), share state between caller and child, or bind the call to a team or task graph are `approximated`; strict mode rejects them.
...
The shared research fixture produced machine-readable reports for fourteen targets: six products lowered statically and verified, eight frameworks executed with real SDK objects and native runners. Every product target preserves the core. Claude Code, Codex, Copilot CLI, and OpenCode were additionally executed headless against scripted model endpoints. In all four, instructions, skill disclosure and activation, and a subagent round trip reached the model. In Copilot CLI the agent file's `agents` list was not enforced and the stdio plugin server ran without its working directory, while `env` and the HTTP header were honored. In Codex the entry instructions travel as a persistent `AGENTS.md` user message, skill activation is a file read rather than a tool, subagents are spawned through a namespace tool whose description lists the project wide agent catalog, and plugin MCP servers reach the main thread only from `.codex/config.toml`, joining a session in the background. In Claude Code the plugin MCP servers reached the model over stdio and streamable HTTP, but the working directory of a stdio server did not, because its MCP configuration has no `cwd` field. In OpenCode 1.18.32 the plugin servers were exposed as function tools, invoked, and the stdio working directory was honored; in OpenCode 2.0.14 the same servers connected but the model was offered no MCP tool, directly or through Code Mode, for a reason not determined from outside. Between those two versions the subagent tool, the skill tool's argument, and MCP exposure all changed, which is why product evidence must be reprobed per release. Seven of eight runtime targets accepted the required core; CrewAI's role, goal, and backstory prompt template approximates both name and instructions, and its custom template override removes role and goal only by collapsing the prompt into a single user message. OpenAI Agents SDK, PydanticAI, and Microsoft Agent Framework accepted the combined draft 0.1 fixture with skill durability and resource access unverified; the other four each failed one or two optional fields, most often description or delegates.
...
- one generic `delegates` field cannot name the mechanism a target actually uses; the narrow `subagents` relation is preserved by Claude Code, OpenCode, Amplifier, and the agent-as-tool frameworks, resolved by Codex and Copilot CLI through project-wide catalogs, unsupported by AFM, and approximated by orchestration-first frameworks;
