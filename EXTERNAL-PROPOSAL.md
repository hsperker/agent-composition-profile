...
- `delegates` conflated agent-as-tool, handoff, graph transition, shared-state execution, and team collaboration. Those mechanisms differ in control, context, and result semantics. One field should not pretend they are one thing. The narrow relation the products agree on, an allowlisted subagent invoked as a bounded task that returns a result, survives as an optional additive `subagents` field.
